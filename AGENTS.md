...
- Alembic migrations: `alembic/versions/` (preserve the validated history; verify with `alembic heads` and `alembic check`)
...
### Backend Domain
- `py314`, mypy `3.14`; Ruff `>=0.14.14,<0.15`; `fail_under = 100`; tuple `except (A, B):`(`no-python2-except`); `lazy="noload"` on all relationships (CI gate exempt `# noload-exempt: <reason>`)(`no-python2-except`); Alembic Windows glob `**/*alembic*/**/*.py`; Ruff S105 `per-file-ignores`; mypy `: ignore[return-value]` unneeded; OpenFeature flagd provider is a runtime dependency and evaluations degrade to explicit call-site defaults if unavailable; `NullSessionBackend` closed prod; `S3Storage` `asyncio.timeout()`.
...
- `uv lock --check` PR locally `main` Dependabot bump -> `git merge origin/main`; `.secrets.baseline` commit; pre-commit any ruff error; Husky v9+: parent-dir paths (`.. allowed`); `setup-husky.cjs` `git config --local core.hooksPath .husky`; pre-commit shell scan raw `docker compose -f docker-compose.full.yml` -> `bash scripts/dc.sh` `pwsh scripts/dc.ps1`; all Actions SHA-pinned; supported Python/Go coverage metrics require 100% per `quality/quality-contract.json`; `actions/upload-artifact@v7` files (`include-hidden-files: true` for `.lighthouseci/` dirs).
...
- Kyverno Policy 9 rejects empty/latest tags; current Caddy baseline has one `frontend:3000` upstream, so any future canary requires a reviewed temporary overlay built from the then-current topology; `lhci collect` Windows EPERM: `lhci-windows-fallback.mjs`; `npm lhci:windows`; LHCI `VITE_LHCI=true`; prod `VITE_LHCI=true`; static dist dir points `dist/client/`; Chrome `--disable-gpu` Linux CI screenshots `--headless=new`; Linux CI CLS `error@0.05`.
