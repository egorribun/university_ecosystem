...
- Backend использует Redis для лимитов и кэша. Укажите `RATE_LIMIT_STORAGE_URI=redis://host:6379/0` и (при необходимости) `CACHE_REDIS_URL`.
- Сервис должен быть доступен при старте: если Redis не отвечает, backend завершит запуск с ошибкой. В docker-compose уже добавлен пример сервиса `redis`.
- Для миграции с `memory://` поднимите Redis в продакшене, обновите переменные окружения и перезапустите pod/контейнер. До успешного подключения приложение не запустится, поэтому проверяйте readiness в k8s/compose.
