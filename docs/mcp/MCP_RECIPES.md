...
| 6 | `gopls-mcp-server` | stdio (`go`) | `go run golang.org/x/tools/gopls@latest mcp` | Go 1.26.4+ SDK toolchain (CI pins 1.26.6) | `go_diagnostics`, `go_file_context`, `go_package_api`, `go_rename_symbol`, `go_search`, `go_symbol_references`, `go_vulncheck`, `go_workspace` | Go language server protocol diagnostics for microservices (`services/ws-hub`, `services/gateway`, `services/file-processor`). |
