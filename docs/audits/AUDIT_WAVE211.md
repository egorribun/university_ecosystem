...
| SW7 | `cdda32f08` | NEW `tests/test_wave211_forward.py` (9: cross-chat-leak 403, not-dest-participant 403, source-not-in-source 404, snapshot copies content+attribution+attachment, reactions-not-copied, multi-forward N→1-commit, dedupe) + 3 integration tests in `test_chat_api.py`. |
...
**0 NEW (z) discoveries** — the AmbiguousForeignKeysError (SW1) + the attribution-None bug (SW7) were caught by the cheap `configure_mappers()` check + the live verify respectively, and fixed within their SW (SAME-mechanism per W138 L#1), not deferred. **0 NEW anti-patterns** (14-pattern register stable post-W159 #15 archival).
