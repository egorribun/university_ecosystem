...
2. **server.js content-sha shift** `f8bd8fab…` → `bd4a3402…` — benign W195-class `.stories.tsx`-addition chunk-graph metadata precedent (size-identical 24,024 b, reproducible × 3, 0 story leakage). The opening prompt §8's "server.js == f8bd8fab" was the W196-W198-stable value; W199 (like W195/W189) shifts it. Main JS **content** byte-identity (the critical invariant) holds exactly — re-verified `1bff1fd7…c97` at final HEAD in the «безупречно?» polish. **Note (polish finding):** the main-chunk FILENAME hash is likewise non-deterministic — a fresh W199 build emits `index-RDoTRau4.js`, not W198's `index-C6pdnyI2.js`, content byte-identical (the same W193 SW5 phenomenon; the 26 new `.stories.tsx` perturb Rolldown's module-graph metadata). Verify the invariant via content sha, never filename. CLAUDE.md's W199 row carried over the W198 filename and was corrected in the polish pass.
