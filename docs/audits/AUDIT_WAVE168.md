...
| 15  | (ARCHIVED W159 SW4) prettier discipline + husky        | ARCHIVED, preserved | **Wave 4 preservation check** — SW1 commit `b633a5333` fired husky pre-commit chain cleanly (lint-staged + prettier --write + eslint --fix + detect-secrets + Python 2 except check ALL PASS). NO `--no-verify` bypass.                                                                                  |
...
| CI Matrix Expansion    | GREEN                               | `gh run watch <run-id>` post SW4 push                                 | ⏳ in progress at audit time (queued; will verify in polish-v1)                                      |
