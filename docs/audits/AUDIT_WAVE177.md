...
| `npm run build` (production) | Main JS `index-BA6s4YBY.js` **179,507 b** — BYTE-IDENTICAL SIZE to W176 baseline `index-BfRpD_fx.js`; new sha `6c069173...` reflects Login chunk relocation; Login route chunk `Login-teulBu_B.js` 24.90 kB carries the SW1 useEffect weight; server.js 23,600 b unchanged; `_shell.html` 66,459 b unchanged; sw.js 53,667 b unchanged | W176 baseline preserved exactly at byte-count level |
| Docker chain | `/healthz 200`, `/login 200/21,669b SSR` (+59 b vs pre-W177 21,610 b — SSR-rendered Login.tsx with new useEffect imports + comment) | W176 baseline +59 b |
...
## Bundle delta vs W176 baseline

**Main JS chunk**: `index-BA6s4YBY.js` **179,507 b** (local Windows build) = W176 baseline `index-BfRpD_fx.js` 179,507 b SIZE EXACTLY. Hash differs due to content change (Login chunk modulepreload graph + entry chunk import bookkeeping). **Login route chunk** `Login-teulBu_B.js` **24.90 kB** — carries the SW1 useEffect code via Vite/Rolldown route-based code-splitting.

**server.js**: **23,600 b** — IDENTICAL to W176 baseline (server-side SSR rendering unchanged structurally).

**_shell.html**: **66,459 b** — IDENTICAL to W176 baseline.

**sw.js**: **53,667 b** — IDENTICAL to W176 baseline (W174 SW2 ensureCsrfCookie auto-fetch already in W174 baseline; no new SW workbox precache entries).

**Cross-platform divergence (W141 polish A3 known invariant)**: Docker Linux container build produced `index-D0EDgCF8.js` post-rebuild; same source code yields different filename hash than local Windows build `index-BA6s4YBY.js` due to Node.js + Rolldown native binary differences. Semantically equivalent. SW4 chrome-devtools-mcp verified the Docker Linux build serves Login.tsx with the W177 SW1 useEffect correctly.
