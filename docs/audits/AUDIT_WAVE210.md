...
- **(z)#1 — MOD-W5-03 `uv lock --check` gate failed because `main` was ahead with an unmerged Dependabot redis bump (NOT "CDN-edge skew" — initial misdiagnosis, corrected per the «безупречно?» self-audit when the PR conflict was resolved).** The `pull_request` event runs against `refs/pull/1126/merge` (egorribun merged onto base `main`). main's Dependabot PR #1127 (`a2161d8c1`) had bumped `pyproject.toml` `redis<8→<9` but **did NOT regenerate `uv.lock`** (Dependabot edits only the manifest) — so main itself was internally inconsistent (pyproject `<9`, lock `<8`). In the merge-context, main's newer pyproject `<9` combined with the stale lock `<8` → `uv lock --check` mismatch. egorribun alone was internally consistent (`<8`/`<8`) → **irreproducible locally** — and **~14 diagnostic rounds** (uv 0.11.7→0.11.17, `--refresh`, `--offline`, `--upgrade`, a Rust-equipped `docker run` mount) were burned chasing a false "CDN-edge skew" hypothesis before the obvious **`git log origin/main ^HEAD`** revealed the `build(deps)` redis commit. The lesson (now a `CLAUDE.md` Gotcha): **when a dep/lock gate fails on a PR but passes locally on the feature branch, the FIRST diagnostic is `git fetch origin main && git log origin/main ^HEAD`** — an unmerged Dependabot manifest bump is the usual culprit. The temporary upload-artifact step (`41201964a`, reverted) → `gh run download` + `diff` correctly showed the **1-line `requires-dist[redis] <8→<9`** drift; `5800cffab` aligned egorribun pyproject `redis<8→<9` + regen lock (resolved redis stays **7.4.0**, metadata-only; `--upgrade`'s 1921-line / 30-package wave correctly rejected); and **`6c3c5ff8c` merging `origin/main`** conclusively resolved it (the post-`5800cffab` `refs/pull/1126/merge` had become a *textual* conflict on the redis line — both branches edited it from base `<8` — which is what flipped the PR to CONFLICTING/DIRTY and why only `workflow_dispatch` runs validated green meanwhile).
