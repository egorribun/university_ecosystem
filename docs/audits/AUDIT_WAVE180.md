...
**Class-wide finding (CORRECTED in polish-v1)**: React #418 affects ALL `ssr: 'data-only'` routes — under BOTH unauth chrome-devtools-mcp context AND real-user auth via wave137-authed-smoke. /messenger (NEW W180 SW3) + /activity + /map (pre-existing since W127 SW6 / W128 SW2) — all 3 emit Minified React error #418 with args=`HTML&args[]=` in browser console.
