...
**Class-wide finding**: React #418 affects ALL `ssr: 'data-only'` routes in unauth chrome-devtools-mcp context (/messenger NEW + /activity + /map pre-existing since W128 SW2). NOT a W180 SW3 regression. Under real-user auth (W179 SW7 wave137-authed-smoke), data-only routes show 0 hydration errors. Documented as W180 §Honesty #1 caveat (characterization-only, not blocking).
...
1. **React #418 hydration error on `ssr: 'data-only'` routes under unauth chrome-devtools-mcp** — affects /messenger NEW W180 SW3 + /activity + /map (pre-existing since W128 SW2). Args=`HTML&args[]=` element-type mismatch class. NOT a W180 SW3 regression. Under real-user auth (W179 SW7 wave137-authed-smoke proved 0 hydration errors on 8 SSR routes). Likely root cause: lazy route component + `ssr: 'data-only'` semantics + unauth route guard behavior interaction. W181+ investigation candidate if/when user reports actual hydration-related UX issue.
