...
- The `root/.env.example` file is a template for local use. Copy it to `root/.env`, replace every secret, and keep the filled version outside of version control. Compose ships with safe development defaults for `DATABASE_URL` and `POSTGRES_PASSWORD_FILE` so commands like `docker compose config` succeed without secrets, but real deployments must override them.
