...
**Status:** concept approved by the repository owner on 2026-08-11; awaiting
review of this written specification before implementation.
...
The workflow fetches that exact object and creates a detached temporary Git
worktree for the base revision. The normal checkout is the candidate/head.
...
| File | Change |
| --- | --- |
| `scripts/quality/compare_paired_benchmarks.py` | Pure parser, pairing, deterministic bootstrap, JSON/Markdown report, and fail-closed CLI. |
...
5. Unit and workflow-contract tests cover every invariant, and actionlint plus
   YAML parsing pass.
6. A fresh hosted PR run validates the new gate before roadmap evidence is
...
performance baseline, does not lower the ten-percent threshold, does not hide
historical charts, and does not provision infrastructure or expose a
self-hosted runner to public PRs.
