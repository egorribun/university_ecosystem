...
- [x] **Step 1: Capture repository and GitHub baseline.**
...
- [x] **Step 2: Reconcile every `Still open`, `Task G.*`, and Phase 0–10/14 item with current code and CI.**
...
- [x] **Step 3: Record only evidence-backed changes in the roadmap.**
...
- [x] **Step 4: Run the roadmap contract tests.**
...
- [ ] **Step 1: Re-run current-HEAD nightly-full-gate and capture each job result/artifact.** Run `31310278607` is queued behind the still-running full mutation job `31307154620`.
- [x] **Step 2: Reproduce each failure locally where platform support exists; install only missing test tools required by the repository's documented commands.**
- [x] **Step 3: Fix the smallest root cause, add or strengthen the corresponding regression/contract test, and rerun the affected job locally.**
- [ ] **Step 4: Verify Pact consumer/provider replay, all four Playwright projects, Checkov blocking behavior, Kyverno policy tests, negative security suites, DAST, load/chaos, and performance thresholds.** DAST and authorized external performance evidence remain unavailable.
- [x] **Step 5: Do not promote an advisory check until the repository has actual green evidence for the documented stabilization window; add an automated window calculator if the current workflow cannot prove it.**
...
- [x] **Step 1: Verify all required status checks against real check-run display names and compare them with the current ruleset.**
- [x] **Step 2: Make Checkov and coverage upload failures observable and blocking without hiding findings behind `soft_fail` or `continue-on-error`.**
- [x] **Step 3: Verify Codecov using the supported current authentication path; if a secret value is genuinely required, add a preflight that fails with an actionable message and record the exact user-only action rather than inventing a token.** OIDC reaches Codecov; repository authorization is the external blocker.
- [x] **Step 4: Validate ownership mapping and bypass-policy documentation with the inventory checker.**
- [x] **Step 5: Recheck the 30-day promotion rule from actual run timestamps; never synthesize historical green days.**
...
- [ ] **Step 1: Run the complete relevant local checks from AGENTS.md plus `git diff --check`, targeted regressions, and final coverage/quality validation.** Final documentation checks and targeted contract tests remain to be run after this update.
