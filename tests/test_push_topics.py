...
def test_normalize_topics():
    ...
    assert pt.normalize_topics(["NEWS", "alerts", "NEWS", "unknown"], allowed_topics=allowed) == ["news", "alerts"]

def test_resolve_topics():
    allowed = ["news", "alerts"]
    assert pt.resolve_topics(None, existing=["alerts", "unknown"], allowed_topics=allowed) == ["alerts"]
    assert pt.resolve_topics(["news"], existing=["alerts"], allowed_topics=allowed) == ["news"]
...
def test_subscription_supports_topic_none_subscription():
    assert pt.subscription_supports_topic(None, "news", allowed_topics=["news"]) is False
...
def test_subscription_supports_topic_user_with_matching_preferences(monkeypatch):
    ...
    class MockState:
        unloaded = set()
    monkeypatch.setattr("sqlalchemy.orm.attributes.instance_state", lambda obj: MockState())

    assert pt.subscription_supports_topic(sub, "news", allowed_topics=["news"]) is True
    assert pt.subscription_supports_topic(sub, "alerts", allowed_topics=["news", "alerts"]) is False
...
def test_subscription_supports_topic_subscription_topics_restriction(monkeypatch):
    ...
    assert pt.subscription_supports_topic(sub, "news", allowed_topics=["news", "alerts"]) is True
    assert pt.subscription_supports_topic(sub, "alerts", allowed_topics=["news", "alerts"]) is False
...
async def test_upsert_user_topics(db_session):
    ...
    # New preference
    topics = await pt.upsert_user_topics(db_session, user_id=user_id, topics=["news"], allowed_topics=allowed)
    assert topics == ["news"]
    
    # Verify in DB
    pref = (await db_session.execute(select(UserPushTopic).where(UserPushTopic.user_id == user_id))).scalar_one()
    assert pref.topics == ["news"]

    # Update existing preference
    topics2 = await pt.upsert_user_topics(db_session, user_id=user_id, topics=["news", "alerts"], allowed_topics=allowed)
...
async def test_synchronize_user_topics(db_session):
    ...
    sub1 = PushSubscription(endpoint="https://sub1", p256dh="dh1", auth="auth1", user_id=user_id, topics=["news"])
    sub2 = PushSubscription(endpoint="https://sub2", p256dh="dh2", auth="auth2", user_id=user_id, topics=["alerts"])
    db_session.add_all([sub1, sub2])
    await db_session.flush()

    # Sync topics
    topics = await pt.synchronize_user_topics(db_session, user_id=user_id, topics=["news", "alerts"], allowed_topics=allowed)
