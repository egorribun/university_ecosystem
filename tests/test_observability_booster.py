...
def test_configure_sentry():
    with patch("app.core.observability.sentry_init") as mock_init, \
...
def test_configure_otel_instrumentation_errors():
    app.core.observability._otel_configured = False
    with patch("app.core.observability.settings") as mock_settings, \
         patch("app.core.observability.SQLAlchemyInstrumentor.instrument", side_effect=RuntimeError), \
         patch("app.core.observability.RedisInstrumentor.instrument", side_effect=RuntimeError), \
         patch("app.core.observability.HTTPXClientInstrumentor.instrument", side_effect=RuntimeError), \
         patch("app.core.observability.OTLPSpanExporter") as mock_span_exporter, \
         patch("app.core.observability.OTLPMetricExporter") as mock_metric_exporter, \
         patch("app.core.observability.OTLPLogExporter") as mock_log_exporter, \
         patch("app.core.observability.BatchSpanProcessor") as mock_batch_span, \
         patch("app.core.observability.PeriodicExportingMetricReader") as mock_metric_reader, \
         patch("app.core.observability.BatchLogRecordProcessor") as mock_batch_log:
...
def test_configure_observability_instrument_app_error():
    ...
    with patch("app.core.observability.settings") as mock_settings, \
         patch("app.core.observability.FastAPIInstrumentor.instrument_app", side_effect=RuntimeError), \
...
async def test_start_worker_monitoring_server_old_uvicorn():
    ...
    with patch("uvicorn.Server", return_value=mock_server):
        stop_fn = await app.core.observability.start_worker_monitoring_server(mock_app, host="127.0.0.1", port=8000)
        await stop_fn()
...
def test_configure_worker_observability():
    with patch("app.core.observability.settings") as mock_settings, \
        ...
        app.core.observability.configure_worker_observability(worker_name="test_worker", engine=MagicMock())
