...
async def test_confirm_totp_enrollment_failure(mock_complete):
    ...
    payload = MagicMock(spec=TotpEnrollmentConfirmIn, enrollment_id="123", code="123456")
    db = AsyncMock()
    ...
    async def mock_get(dep, *a, **kw):
        ...
        if 'AuditService' in str(dep): return audit
...
@patch("app.services.webauthn.WebAuthnService.verify_registration", new_callable=AsyncMock)
async def test_confirm_webauthn_registration_failure(mock_verify, mock_get_challenge):
    ...
    payload = MagicMock(spec=WebAuthnRegistrationVerifyIn, challenge="token", response={}, label="mykey")
    db = AsyncMock()
    audit = AsyncMock()
    user = MagicMock(id="user_123")
