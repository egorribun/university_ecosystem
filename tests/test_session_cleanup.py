...
async def test_revoke_sessions_matching():
    ...
    session_2 = ActiveSession(revoked_at=dt.datetime.now(dt.UTC), jti="jti2")
    ...
    with patch(
        ...
        res = await revoke_sessions_matching(
            db=db, whereclause=whereclause, rotate_signing_key=False
        )
        assert res == 1
...
async def test_cleanup_expired_sessions():
    ...
    with patch(
        "app.services.session_cleanup.delete_sessions_matching", return_value=0
    ) as mock_delete:
        res = await cleanup_expired_sessions(db=db)
        assert res == 0
    ...
    with (
        patch("app.services.session_cleanup.async_session") as mock_session,
        patch(
            "app.services.session_cleanup.delete_sessions_matching", return_value=3
        ) as mock_delete,
    ):
...
async def test_start_session_cleanup_scheduler_error():
    ...
    with (
        ...
        patch("app.services.session_cleanup.asyncio.sleep", side_effect=mock_sleep_fn),
    ):
