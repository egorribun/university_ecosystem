...
@pytest.mark.asyncio
@patch("app.services.auth.graphql_token_validator.get_cache_client", new_callable=AsyncMock)
...
async def test_check_fingerprint_exception(mock_fp_service, validator):
    ...
    instance.validate_fingerprint = AsyncMock(side_effect=Exception("Fingerprint failed"))
