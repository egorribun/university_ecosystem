...
from app.core.exceptions.domain import BusinessRuleViolation, EntityAlreadyExists, EntityNotFound, PermissionDenied
from app.models import ActiveSession, Event, EventAttendance, Notification, User, UserProfile
...
async def test_upload_avatar_success(mock_uow, monkeypatch):
    ...
    monkeypatch.setattr("app.services.user.media_service.delete_static_file", mock_delete)
    
    mock_update = MagicMock()
    monkeypatch.setattr("app.services.user.media_service.update_user_attributes", mock_update)
...
async def test_analytics_get_attendance_stats(mock_db, monkeypatch):
    ...
    monkeypatch.setattr("app.services.user.analytics_service.stats_cache.get_cached_stats", mock_get_cached)
    monkeypatch.setattr("app.services.user.analytics_service.stats_cache.set_cached_stats", mock_set_cached)
...
async def test_admin_delete_user(mock_uow, monkeypatch):
    ...
    monkeypatch.setattr("app.services.user.compliance_service.anonymize_user_data", mock_anonymize)
...
async def test_admin_delete_user_not_admin(mock_uow):
    ...
    with pytest.raises(PermissionDenied):
        await svc.admin_delete_user(uuid.uuid4(), MagicMock(), MagicMock(role="student"))
...
async def test_stats_get_grade_stats(monkeypatch):
    ...
    mock_get_cached = AsyncMock(return_value=None)
    mock_set_cached = AsyncMock()
    monkeypatch.setattr("app.services.user.stats_service.stats_cache.get_cached_stats", mock_get_cached)
    monkeypatch.setattr("app.services.user.stats_service.stats_cache.set_cached_stats", mock_set_cached)
