...
async def test_register_key_with_tags(monkeypatch):
    ...
    mock_pipe.sadd.assert_called_once_with(f"{CacheTag.SCHEDULE}:keys", "schedule:group:1")
...
async def test_invalidates_cache_decorator(monkeypatch):
    ...
    monkeypatch.setattr("app.services.cache_invalidation.invalidate_by_tag", mock_invalidate)
