...
async def test_request_timeout_extension_timeout():
    ...
    try:
        ...
        with pytest.raises(GraphQLError, match="Request exceeded the maximum execution time"):
            await gen.athrow(e)
...
async def test_query_cost_extension_user_rate_limit():
    ...
    with patch("app.graphql.extensions.visit", side_effect=mock_visit_fn), \
         patch("app.graphql.extensions._increment_user_cost", new_callable=AsyncMock) as mock_incr:
