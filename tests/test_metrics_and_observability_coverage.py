...
def test_record_background_task_error():
    ...
    val = REGISTRY.get_sample_value("background_task_errors_total", {"task_name": "my_test_task"})
...
def test_record_login_failure():
    ...
    val = REGISTRY.get_sample_value("auth_login_failure_total", {"reason": "invalid_credentials"})
    assert val is not None
    assert val >= 1.0
...
def test_record_notification_failed():
    ...
    val = REGISTRY.get_sample_value("notifications_failed_total", {"type": "sms", "reason": "timeout"})
    assert val is not None
    assert val >= 1.0
...
def test_record_presence_event():
    ...
    val = REGISTRY.get_sample_value("websocket_presence_events_total", {"state": "online", "source": "web"})
    assert val is not None
    assert val >= 1.0

def test_record_presence_throttled():
    m.record_presence_throttled("away", "mobile")
    val = REGISTRY.get_sample_value("websocket_presence_throttled_total", {"state": "away", "source": "mobile"})
    assert val is not None
    assert val >= 1.0
...
def test_websocket_connections():
    ...
    assert REGISTRY.get_sample_value("websocket_connections_active", {"path": "/ws/chat"}) == 5.0
    m.inc_ws_connections("/ws/chat")
    assert REGISTRY.get_sample_value("websocket_connections_active", {"path": "/ws/chat"}) == 6.0
...
def test_record_circuit_breaker():
    ...
    assert REGISTRY.get_sample_value("circuit_breaker_state", {"service": "clamav"}) is not None
    m.record_circuit_breaker_trip("clamav")
    assert REGISTRY.get_sample_value("circuit_breaker_trips_total", {"service": "clamav"}) >= 1.0

def test_record_health_probe():
    m.record_health_probe("db", "ok", 0.05)
    assert REGISTRY.get_sample_value("healthcheck_status_total", {"component": "db", "status": "ok"}) >= 1.0

def test_record_redis_command():
    m.record_redis_command("GET", 0.002, success=True)
    assert REGISTRY.get_sample_value("redis_command_duration_seconds_count", {"command": "GET"}) >= 1.0
    m.record_redis_command("SET", 0.005, success=False)
    assert REGISTRY.get_sample_value("redis_command_errors_total", {"command": "SET"}) >= 1.0

def test_record_db_operation():
    m.record_db_operation("select", 0.01, success=True)
    assert REGISTRY.get_sample_value("db_operation_duration_seconds_count", {"operation": "select"}) >= 1.0
    m.record_db_operation("insert", 0.02, success=False)
    assert REGISTRY.get_sample_value("db_operation_errors_total", {"operation": "insert"}) >= 1.0

def test_resolve_path_template():
    assert m._resolve_path_template("/api/v1/users/123/profile") == "/api/v1/users/{id}/profile"
    assert m._resolve_path_template("/api/v1/news/456") == "/api/v1/news/{id}"
    assert m._resolve_path_template("/static/images/avatar.png") == "/static/images/{id}"
...
def test_periodic_task_metrics():
    ...
    run = obs.PeriodicTaskRun(task_name="test_periodic", duration_seconds=1.5, success=True)
    ptm.record_run(run)
    
    val = REGISTRY.get_sample_value("periodic_task_duration_seconds_count", {"task_name": "test_periodic", "success": "true"})
    assert val is not None
    assert val >= 1.0
