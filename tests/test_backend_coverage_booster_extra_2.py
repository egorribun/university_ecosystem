...
async def test_complete_passkey_enrollment_invalid_payload():
    ...
    with patch("app.api.auth.mfa.mfa.get_challenge", AsyncMock(return_value=mock_challenge)):
        with pytest.raises(HTTPException) as exc:
            await mfa_api.confirm_webauthn_registration(payload=payload, request=request, user=user)
        assert exc.value.status_code == 400
...
async def test_complete_passkey_enrollment_success():
    ...
    with patch("app.api.auth.mfa.mfa.get_challenge", AsyncMock(return_value=mock_challenge)), \
        ...
        res = await mfa_api.confirm_webauthn_registration(payload=payload, request=request, user=user)
...
async def test_get_online_users_for_user():
    ...
    with patch("app.api.ws.presence._get_presence_audience", AsyncMock(return_value=[target_id1, target_id2])), \
...
async def test_websocket_chat_update_last_seen_exception():
    ...
    with patch("app.api.ws.authenticator.authenticator.authenticate_upgrade", AsyncMock(return_value=(mock_user, "jti", "subprotocol"))), \
         patch("app.api.websocket.manager") as mock_manager, \
         patch("app.api.websocket._update_last_seen", AsyncMock()) as mock_update_last_seen:
