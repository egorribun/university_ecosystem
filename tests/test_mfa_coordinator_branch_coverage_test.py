...
@pytest.mark.asyncio
@patch("app.services.auth.mfa_coordinator.MfaCoordinator._resolve_mfa_capabilities", return_value={})
@patch("app.services.auth.mfa_coordinator.MfaCoordinator._collect_mfa_challenges", return_value=[])
async def test_check_and_issue_challenges_no_methods(mock_collect, mock_resolve, coordinator):
...
@pytest.mark.asyncio
@patch("app.services.auth.mfa_coordinator.MfaCoordinator._resolve_mfa_capabilities", return_value={"totp": True})
@patch("app.services.auth.mfa_coordinator.MfaCoordinator._collect_mfa_challenges", return_value=["totp"])
async def test_check_and_issue_challenges_with_response(mock_collect, mock_resolve, coordinator):
    user = MagicMock(id="123", mfa_required=True, mfa_default_method=None)
    response = MagicMock(spec=Response)
    
    res = await coordinator.check_and_issue_challenges(user, AsyncMock(), response, "en")
...
@patch("app.auth.mfa.describe_challenge_attempts", return_value=(1, 5, 4))
@patch("app.services.webauthn.WebAuthnService.get_authentication_options", new_callable=AsyncMock)
async def test_collect_mfa_challenges_webauthn(mock_options, mock_describe, mock_issue, coordinator):
    user = MagicMock(id="123")
