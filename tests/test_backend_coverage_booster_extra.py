...
async def test_mfa_confirm_totp_enrollment_failure():
    ...
    with patch("app.auth.mfa.complete_totp_enrollment", side_effect=HTTPException(400, "invalid_code")):
        with pytest.raises(HTTPException) as exc:
...
async def test_mfa_start_totp_enrollment_endpoint():
    ...
    with patch("app.models.user_loaders.ensure_mfa_relationships_loaded", AsyncMock()), \
         patch("app.auth.mfa.start_totp_enrollment", AsyncMock(return_value=(enrollment, "secret", "url"))):
...
def test_localized_notification_field_required():
    assert notifications_api._localized_notification_field("en", None, None, required=True) == ""
    assert notifications_api._localized_notification_field("en", "RU", None, required=True) == "RU"
    assert notifications_api._localized_notification_field("en", None, "EN", required=True) == "EN"
...
async def test_list_notifications_bad_cursor():
    ...
    response = MagicMock()
    with pytest.raises(HTTPException) as exc:
        await notifications_api.list_notifications(response, db, user=user, cursor="invalid_base64_string")
    assert exc.value.status_code == 400

    bad_cursor = base64.b64encode(b"2026-01-01T00:00:00,not-a-uuid").decode()
    with pytest.raises(HTTPException) as exc:
        await notifications_api.list_notifications(response, db, user=user, cursor=bad_cursor)
    assert exc.value.status_code == 400

@pytest.mark.asyncio
async def test_fetch_notification_rows_fallback_empty():
    db = AsyncMock()
    with patch("app.api.notifications._existing_notification_columns", AsyncMock(return_value=set())):
        res, cols = await notifications_api._fetch_notification_rows_fallback(db, "user-id", limit=10, cursor_info=None)
        assert res == []
        assert cols == set()
...
async def test_subscribe_integrity_error_retry():
    ...
    with patch("app.routers.notifications.enforce_rate_limit", AsyncMock()), \
         ...
         patch("app.routers.notifications.resolve_topics", MagicMock(return_value={"general"})):
...
async def test_update_subscription_topics_not_found():
    ...
    payload = PushSubscriptionTopicsUpdate(endpoint="https://some-endpoint", topics=["general"])
    request = MagicMock()
    db = AsyncMock()
    user = MagicMock()
    user.id = uuid.uuid4()
...
async def test_now_playing_endpoint_error_states():
    ...
    with patch("app.api.spotify._ensure_access_token", AsyncMock(return_value="token")), \
         patch("app.api.spotify._spotify_http_client.get", AsyncMock(return_value=Response(204))):
        res = await spotify_api.now_playing(request=request, db=db, user=user)
        assert res.status_code == 204
        assert not user.spotify.is_playing

    with patch("app.api.spotify._ensure_access_token", AsyncMock(return_value="token")), \
         patch("app.api.spotify._spotify_http_client.get", AsyncMock(return_value=Response(401))):
        with pytest.raises(HTTPException) as exc:
    ...
    with patch("app.api.spotify._ensure_access_token", AsyncMock(return_value="token")), \
         patch("app.api.spotify._spotify_http_client.get", AsyncMock(return_value=resp_429)):
        with pytest.raises(HTTPException) as exc:
            await spotify_api.now_playing(request=request, db=db, user=user)
        assert exc.value.status_code == 429
    ...
    with patch("app.api.spotify._ensure_access_token", AsyncMock(return_value="token")), \
         patch("app.api.spotify._spotify_http_client.get", side_effect=CircuitBreakerOpenError("open", remaining_seconds=10.0, failure_count=5)):
        res = await spotify_api.now_playing(request=request, db=db, user=user)
        assert not res.is_playing
...
def test_configure_metrics_placeholder_credentials():
    app = MagicMock()
    app.state = MagicMock()
    setattr(app.state, "_metrics_configured", False)
...
async def test_websocket_chat_connect_rejected():
    ...
    with patch("app.api.ws.authenticator.authenticator.authenticate_upgrade", AsyncMock(return_value=(user, session_jti, None))), \
...
def test_enforce_profile_cache_integrity_errors():
    with patch("app.api.users.settings") as mock_settings:
        ...
        envelope = {
            "version": 1,
            "expiresAt": int((datetime.now(UTC) + timedelta(minutes=5)).timestamp() * 1000),
            "data": "some_data",
            "signature": "bad_sig"
        }
        req4 = MagicMock()
