"""Tests for app/tasks — cleanups.py, notifications.py, email.py

Calls the inner logic of each @broker.task decorated function directly
(bypassing NATS dispatch) to boost coverage of the tasks layer.
"""
...
class TestCleanupNotificationsTask:
    ...
    async def test_runs_when_retention_days_positive(self):
        ...
        with patch("app.tasks.cleanups.settings") as mock_settings, \
             patch("app.tasks.cleanups.cleanup_stale_notifications", new_callable=AsyncMock) as mock_cleanup:
            mock_settings.notifications_retention_days = 30

            # Call the inner function directly (unwrap the @broker.task decorator)
            inner = cleanups.cleanup_notifications_task.__wrapped__ if hasattr(
                cleanups.cleanup_notifications_task, "__wrapped__"
            ) else cleanups.cleanup_notifications_task.original_func if hasattr(
                cleanups.cleanup_notifications_task, "original_func"
    ...
    async def test_skips_when_retention_days_zero(self):
        ...
        with patch("app.tasks.cleanups.settings") as mock_settings, \
            ...
            inner = getattr(cleanups.cleanup_notifications_task, "__wrapped__",
...
class TestCleanupSessionsTask:
    ...
    async def test_calls_cleanup_expired_sessions(self):
        ...
        with patch("app.tasks.cleanups.cleanup_expired_sessions", new_callable=AsyncMock) as mock_cleanup:
            inner = getattr(cleanups.cleanup_sessions_task, "__wrapped__",
...
class TestCleanupStoriesTask:
    ...
    async def test_runs_when_enabled(self):
        ...
        with patch("app.tasks.cleanups.settings") as mock_settings, \
             patch("app.tasks.cleanups.cleanup_expired_stories", new_callable=AsyncMock) as mock_cleanup:
            mock_settings.stories_cleanup_enabled = True
            inner = getattr(cleanups.cleanup_stories_task, "__wrapped__",
...
class TestCleanupPasswordResetTokensTask:
    ...
    async def test_calls_cleanup_stale_password_reset_tokens(self):
        ...
        with patch("app.tasks.cleanups.settings") as mock_settings, \
             patch("app.tasks.cleanups.cleanup_stale_password_reset_tokens", new_callable=AsyncMock) as mock_cleanup:
            mock_settings.password_reset_cleanup_retention_minutes = 60
            inner = getattr(cleanups.cleanup_password_reset_tokens_task, "__wrapped__",
...
class TestCleanupEmailChangeTokensTask:
    ...
    async def test_calls_cleanup_stale_email_change_tokens(self):
        ...
        with patch("app.tasks.cleanups.settings") as mock_settings, \
             patch("app.tasks.cleanups.cleanup_stale_email_change_tokens", new_callable=AsyncMock) as mock_cleanup:
            mock_settings.email_change_cleanup_retention_minutes = 120
            inner = getattr(cleanups.cleanup_email_change_tokens_task, "__wrapped__",
...
class TestCleanupMfaChallengesTask:
    ...
    async def test_calls_cleanup_stale_mfa_challenges(self):
        ...
        with patch("app.tasks.cleanups.cleanup_stale_mfa_challenges", new_callable=AsyncMock) as mock_cleanup:
            inner = getattr(cleanups.cleanup_mfa_challenges_task, "__wrapped__",
...
class TestCleanupPrivacyArtifactsTask:
    ...
    async def test_calls_cleanup_privacy_artifacts(self):
        ...
        with patch("app.tasks.cleanups.settings") as mock_settings, \
             patch("app.tasks.cleanups.cleanup_privacy_artifacts", new_callable=AsyncMock) as mock_cleanup:
            ...
            inner = getattr(cleanups.cleanup_privacy_artifacts_task, "__wrapped__",
...
class TestManagePartitionsTask:
    ...
    async def test_runs_when_enabled(self):
        ...
        with patch("app.tasks.cleanups.settings") as mock_settings, \
             patch("app.tasks.cleanups.ensure_partitions_exist", new_callable=AsyncMock) as mock_ensure:
            mock_settings.partition_management_enabled = True
            inner = getattr(cleanups.manage_partitions_task, "__wrapped__",
...
class TestCleanupDeadLetterJobsTask:
    ...
    async def test_runs_when_retention_positive(self):
        ...
        with patch("app.tasks.cleanups.settings") as mock_settings, \
            ...
            inner = getattr(cleanups.cleanup_dead_letter_jobs_task, "__wrapped__",
...
class TestSendAuthEmail:
    def test_calls_send_reset_email(self):
        ...
        with patch("app.tasks.email.send_reset_email") as mock_send:
            inner = getattr(email_tasks.send_auth_email, "__wrapped__",
                            email_tasks.send_auth_email)
            if callable(inner):
                inner("user@example.com", "https://example.com/reset", "John Doe", locale="en")
...
class TestSendLockoutAlert:
    def test_calls_send_lockout_email(self):
        ...
        with patch("app.utils.email.send_lockout_email") as mock_send:
            inner = getattr(email_tasks.send_lockout_alert, "__wrapped__",
                            email_tasks.send_lockout_alert)
            if callable(inner):
                inner("user@example.com", "Jane Doe", locale="ru")
            mock_send.assert_called_once_with("user@example.com", "Jane Doe", locale="ru")
...
class TestEnqueueNewsNotificationTask:
    ...
    async def test_notifies_when_news_found(self):
        ...
        with patch("app.tasks.notifications.async_session") as mock_cm, \
             patch("app.tasks.notifications._notify_about_news", new_callable=AsyncMock) as mock_notify:
            ...
            inner = getattr(notif_tasks.enqueue_news_notification_task, "__wrapped__",
...
class TestEnqueueEventNotificationTask:
    ...
    async def test_notifies_when_event_found(self):
        ...
        with patch("app.tasks.notifications.async_session") as mock_cm, \
             patch("app.tasks.notifications._notify_about_event", new_callable=AsyncMock) as mock_notify:
            mock_cm.return_value.__aenter__ = AsyncMock(return_value=mock_db)
            mock_cm.return_value.__aexit__ = AsyncMock(return_value=False)

            inner = getattr(notif_tasks.enqueue_event_notification_task, "__wrapped__",
...
class TestEnqueueCommentNotificationTask:
    ...
    async def test_notifies_when_all_objects_found(self):
        ...
        with patch("app.tasks.notifications.async_session") as mock_cm, \
             patch("app.tasks.notifications._notify_about_comment", new_callable=AsyncMock) as mock_notify:
            mock_cm.return_value.__aenter__ = AsyncMock(return_value=mock_db)
            mock_cm.return_value.__aexit__ = AsyncMock(return_value=False)

            inner = getattr(notif_tasks.enqueue_comment_notification_task, "__wrapped__",
                            notif_tasks.enqueue_comment_notification_task)
            if hasattr(inner, "__call__"):
                await inner(news_id=news_id, comment_id=comment_id, user_id=user_id, locale="en")

            mock_notify.assert_called_once_with(mock_db, mock_news, mock_comment, mock_author, locale="en")
