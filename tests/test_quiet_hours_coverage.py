...
def test_is_user_in_quiet_hours_current_time():
    prefs = MockPreferences(
        dnd_enabled=True, dnd_start=dt.time(22, 0), dnd_end=dt.time(6, 0)
    )
    user = MockUser(preferences=prefs)

    with patch(
        "app.services.notifications.quiet_hours._current_local_time"
    ) as mock_curr:
...
def test_prepare_push_payload_for_user():
    ...
    payload = {"title": "Hello", "body": "World", "data": {"foo": "bar"}}
    ...
    payload_no_data = {"title": "Hello", "body": "World"}
    res_no_data = prepare_push_payload_for_user(
        payload_no_data, user, now_time=dt.time(23, 0)
    )
