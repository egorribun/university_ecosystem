...
func main() {
	...
	// NewClient creates a virtual connection that connects on first RPC
	grpcConn, err := grpc.NewClient(cfg.FileProcessorAddr,
		grpc.WithTransportCredentials(insecure.NewCredentials()),
	)
	if err != nil {
		// Log but don't fail hard, connection issues will surface on first RPC
		logger.Warn("Failed to create File Processor gRPC client", zap.Error(err))
	} else {
		defer func() { _ = grpcConn.Close() }()
		logger.Info("Created File Processor gRPC client", zap.String("addr", cfg.FileProcessorAddr))
	}
