...
func TestEmpirical_MaxClientsPreCheck(t *testing.T) {
	...
	require.NoError(t, rdb.Set(ctx, ticketKey, "user-maxclient-test:jti-123:tenant-1", 15*time.Second).Err())
	...
	t.Run("Allows upgrade when hub capacity is not reached", func(t *testing.T) {
		...
		require.NoError(t, rdb.Set(ctx, wsTicketKeyPrefix+ticket2, "user-maxclient-test2:jti-456:tenant-1", 15*time.Second).Err())
