...
def detect_mime_type(data: bytes) -> str | None:
    ...
    if _magic_mime_detector is _MAGIC_NOT_INITIALIZED:
        ...
        except Exception:  # RZ-22-01-JUSTIFIED: optional dependency — libmagic may not be installed (reviewed TD-27-04)
    ...
    if detector is not None:
        try:
            ...
            except Exception:  # RZ-22-01-JUSTIFIED: optional dependency — libmagic API varies by version (reviewed TD-27-04)
                logger.warning("libmagic failed to detect MIME type", exc_info=True)
                result = None
        except Exception:  # RZ-22-01-JUSTIFIED: optional dependency — libmagic may raise various errors (reviewed TD-27-04)
            logger.warning("libmagic failed to detect MIME type", exc_info=True)
            result = None
