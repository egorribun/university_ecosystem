...
@cached(prefix="schedule:group", key_builder=lambda db, group_id: str(group_id))
...
async def toggle_news_like(db: AsyncSession, news_id: int, user_id: int) -> bool:
    """Toggle a like for a news item. Returns True if liked, False if unliked."""
    ...
    if like:
        ...
        new_like = models.NewsLike(news_id=news_id, user_id=user_id)
        db.add(new_like)
        await db.commit()
        return True
...
async def create_news_comment(
    db: AsyncSession, news_id: int, user_id: int, content: str
) -> models.NewsComment:
