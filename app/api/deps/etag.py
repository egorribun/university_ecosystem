...
def cached_endpoint(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if_none_match = kwargs.get("if_none_match") or request.headers.get("if-none-match")
            ...
            if cache.enabled and request.method in ("GET", "HEAD"):
                ...
                cacheable_kwargs = {
                    ...
                    if k not in ("request", "response", "user", "db", "events", "checker", "if_none_match", "background", "notifications")
                }
            ...
            if is_pydantic:
                serialized_bytes = payload.model_dump_json(by_alias=True).encode("utf-8")
            elif isinstance(payload, list) and len(payload) > 0 and hasattr(payload[0], "model_dump_json"):
                # Handle lists of Pydantic models
                # Construct JSON array string manually to keep Rust speed for inner objects
                inner = ",".join(item.model_dump_json(by_alias=True) for item in payload)
                serialized_bytes = f"[{inner}]".encode("utf-8")
            else:
            ...
            if cache.enabled and cache_key:
                ...
                final_response = Response(content=serialized_bytes, media_type="application/json")
