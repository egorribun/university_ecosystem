...
class ActiveSession(Base, UUID7PrimaryKeyMixin, UserFK):
    ...
    signing_key: Mapped[str] = mapped_column(String, nullable=False, default=_generate_session_signing_key)
    mfa_required: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False, index=True)
    mfa_completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    mfa_method: Mapped[str | None] = mapped_column(String(64))
    mfa_verified_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
