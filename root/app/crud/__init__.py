...
async def get_user_auth(db: AsyncSession, login: str):
    stmt = select(models.User).where(func.lower(models.User.email) == login.lower())
    result = await db.execute(stmt)
    return result.scalar_one_or_none()

async def create_user(db: AsyncSession, user_in: schemas.UserCreate):
    code = None
    if hasattr(user_in, "invite_code") and getattr(user_in, "role", "student") in ("teacher", "admin"):
        ...
        if not code:
            raise Exception("Неверный или уже использованный invite code")
    exists = await db.execute(select(models.User).where(func.lower(models.User.email) == user_in.email.lower()))
...
async def _files_by_event(db: AsyncSession, event_ids: List[int]) -> Dict[int, List[models.EventFile]]:
    if not event_ids:
        return {}
    rows = await db.execute(select(models.EventFile).where(models.EventFile.event_id.in_(event_ids)))
...
async def get_all_events(
    ...
    now = datetime.utcnow()
    if search:
        q = q.where(or_(models.Event.title.ilike(f"%{search}%"), models.Event.description.ilike(f"%{search}%")))
    if type:
    ...
    if is_active:
        ...
        q = q.where(models.Event.ends_at < now)
    events = (await db.execute(q.order_by(models.Event.starts_at.asc()))).scalars().all()
    ids = [e.id for e in events]
    counts = await _attendance_counts(db, ids)
    files_map = await _files_by_event(db, ids)
    registered_ids = set()
    if user_id:
        reg_q = await db.execute(select(models.EventAttendance.event_id).where(models.EventAttendance.user_id == user_id))
...
async def register_attendance(db: AsyncSession, data: schemas.EventAttendanceCreate, user_id: int):
    stmt = select(models.EventAttendance).where(
        and_(models.EventAttendance.event_id == data.event_id, models.EventAttendance.user_id == user_id)
    )
    ...
    record = models.EventAttendance(user_id=user_id, event_id=data.event_id, qr_code=qr_code)
    db.add(record)
    await db.commit()
    await db.refresh(record)
    return record

async def unregister_attendance(db: AsyncSession, data: schemas.EventAttendanceCreate, user_id: int):
    stmt = select(models.EventAttendance).where(
        and_(models.EventAttendance.event_id == data.event_id, models.EventAttendance.user_id == user_id)
    )
...
async def get_my_events(db: AsyncSession, user_id: int):
    ids = (await db.execute(select(models.EventAttendance.event_id).where(models.EventAttendance.user_id == user_id))).scalars().all()
    if not ids:
        return []
...
async def get_schedule_by_group(db: AsyncSession, group_id: int):
    result = await db.execute(select(models.Schedule).where(models.Schedule.group_id == group_id).order_by(models.Schedule.weekday, models.Schedule.start_time))
    return result.scalars().all()
...
async def get_users(db: AsyncSession, group_id: Optional[int] = None, full_name: Optional[str] = None, role: Optional[str] = None) -> List[models.User]:
...
async def admin_update_user(db: AsyncSession, user_id: int, data: schemas.UserAdminUpdate) -> models.User:
    user = await db.get(models.User, user_id)
    if not user:
        raise Exception("Пользователь не найден")
    for field in ["full_name", "email", "role", "group_id"]:
