...
from app.deps.cache import get_cache, RedisCache
...
class SessionBackend(ABC):
    ...
    async def register_session(
        self, user_id: int, jti: str, expires_at: datetime, metadata: dict[str, Any] | None = None
    ) -> None:
        pass
...
async def get_session_backend() -> SessionBackend:
    ...
    class NullSessionBackend(SessionBackend):
        async def register_session(self, *args, **kwargs) -> None: pass
        async def is_session_valid(self, jti: str) -> bool: return True
        async def revoke_session(self, jti: str) -> None: pass
