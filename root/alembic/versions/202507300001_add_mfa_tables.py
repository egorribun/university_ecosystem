...
def upgrade() -> None:
    ...
    if _USERS_TABLE in tables:
        ...
        if (
            ...
            and "mfa_recovery_codes_generated_at" in _column_names(sa.inspect(bind), _USERS_TABLE)
        ):
            op.create_index(
    ...
    if _ACTIVE_SESSIONS_TABLE in tables:
        ...
        if (
            ...
            and "mfa_required" in _column_names(sa.inspect(bind), _ACTIVE_SESSIONS_TABLE)
        ):
            op.create_index(
        ...
        if (
            ...
            and "mfa_completed_at" in _column_names(sa.inspect(bind), _ACTIVE_SESSIONS_TABLE)
        ):
            op.create_index(
