...
def upgrade() -> None:
    ...
    if "push_subscriptions" not in tables:
        ...
        op.create_index(op.f("ix_push_subscriptions_created_at"), "push_subscriptions", ["created_at"])
        op.create_index(op.f("ix_push_subscriptions_last_seen_at"), "push_subscriptions", ["last_seen_at"])
    else:
        cols = {c["name"] for c in inspector.get_columns("push_subscriptions")}
        if "topics" not in cols:
            op.add_column("push_subscriptions", sa.Column("topics", sa.JSON, nullable=False, server_default=sa.text("'[]'")))
        if "created_at" not in cols:
            op.add_column("push_subscriptions", sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False))
        if "last_seen_at" not in cols:
            op.add_column("push_subscriptions", sa.Column("last_seen_at", sa.DateTime(timezone=True)))
        ...
        if not _index_exists(inspector, "push_subscriptions", op.f("ix_push_subscriptions_user_id")):
            op.create_index(op.f("ix_push_subscriptions_user_id"), "push_subscriptions", ["user_id"])
        endpoint_ix_name = op.f("ix_push_subscriptions_endpoint")
        if not _index_exists(inspector, "push_subscriptions", endpoint_ix_name):
            op.create_index(endpoint_ix_name, "push_subscriptions", ["endpoint"], unique=True)
        if not _index_exists(inspector, "push_subscriptions", op.f("ix_push_subscriptions_created_at")):
            op.create_index(op.f("ix_push_subscriptions_created_at"), "push_subscriptions", ["created_at"])
        if not _index_exists(inspector, "push_subscriptions", op.f("ix_push_subscriptions_last_seen_at")):
            op.create_index(op.f("ix_push_subscriptions_last_seen_at"), "push_subscriptions", ["last_seen_at"])
