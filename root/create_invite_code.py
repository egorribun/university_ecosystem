...
async def create_invite_code(role: str):
    ...
    async with async_session() as session:
        invite = InviteCode(code=code, role=role, is_active=True, created_at=datetime.now(UTC))
        session.add(invite)
...
if __name__ == "__main__":
    role = input("Для какой роли сгенерировать invite-код? (teacher/admin): ").strip().lower()
