...
from app.api.chat import _encode_cursor, _decode_cursor
...
async def test_get_chats_list_simple(async_client, user_factory, db_session):
    ...
    for c in chats: await db_session.refresh(c)
...
async def test_send_message_errors(async_client, user_factory, db_session, monkeypatch):
    ...
    # 1. Non-existent chat
    resp = await async_client.post(
        "/chats/non-existent-id/messages",
        data={"content": "Hello"},
        headers=headers
    )
    assert resp.status_code == 404
    ...
    assert resp.status_code == 400
    assert "Too many attachments" in str(resp.content) or "too many" in str(resp.content).lower() # Relaxed check for translation keys
...
async def test_mark_read_logic(async_client, user_factory, db_session):
    ...
    assert msg3.read_status is False # Should not affect my own messages technically, or logic ignores them
