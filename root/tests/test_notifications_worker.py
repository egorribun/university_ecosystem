...
from app.workers.notifications import NotificationsScheduler, start_notifications_scheduler
...
async def test_scheduler_run_once(db_session):
    ...
    with patch("app.workers.notifications.generate_schedule_reminders", new_callable=AsyncMock) as mock_gen:
        ...
        with patch("app.workers.notifications.async_session", return_value=mock_db_ctx):
            scheduler = NotificationsScheduler(poll_seconds=1, window_minutes=1, max_backoff_seconds=1)
