...
from app.services.notification_templates import render_notification_template, ScenarioContext
...
async def test_user_service_basics():
    ...
    with patch("app.services.user_service.delete_static_file", new_callable=AsyncMock) as m_del, \
         patch("app.services.user_service.ensure_mfa_relationships_loaded", new_callable=AsyncMock):
...
async def test_auth_service_basics():
    ...
    # Change password
    data = schemas.UserPasswordChangeIn(current_password="old", new_password="newpassword123")
    with patch("app.services.auth_service.verify_password") as m_verify, \
