...
async def analyze_query_plan(session: AsyncSession, query: str) -> dict:
    ...
    if not rows or not rows[0]:
        return {"plan": None, "uses_index": False, "seq_scan": True, "estimated_cost": float("inf")}
...
class TestCriticalQueryPlans:
    ...
    async def test_user_by_email_uses_index(self):
        ...
        async with async_session() as session:
            ...
            assert plan["uses_index"] or plan["node_type"] in ("Index Scan", "Index Only Scan", "Bitmap Index Scan"), \
