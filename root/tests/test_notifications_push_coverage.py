...
async def test_subscribe_flow(mock_db, mock_user, mock_request):
    ...
    with patch("app.routers.notifications.ensure_push_subscription_schema", AsyncMock()), \
    ...
    existing = models.PushSubscription(id=1, user_id=99, endpoint="https://push.com/123")
...
async def test_unsubscribe(mock_db, mock_user, mock_request):
    ...
    # Found and removed
    existing = models.PushSubscription(id=1, user_id=mock_user.id, endpoint="https://push.com/123")
    mock_db.execute.return_value = MagicMock(scalar_one_or_none=MagicMock(return_value=existing))
    
    with patch("app.routers.notifications._refresh_user_topic_preferences", AsyncMock()):
        res = await unsubscribe(payload, mock_request, mock_db, mock_user)
...
async def test_update_topics(mock_db, mock_user, mock_request):
    payload = PushSubscriptionTopicsUpdate(endpoint="https://push.com/123", topics=["sports"])
    ...
    existing = models.PushSubscription(
        ...
        topics=["news"], p256dh="key", auth="secret"
    )
    mock_db.execute.return_value = MagicMock(scalar_one_or_none=MagicMock(return_value=existing))
    
    with patch("app.routers.notifications.synchronize_user_topics", AsyncMock(return_value=["sports"])), \
         patch("app.routers.notifications.normalize_topics", side_effect=lambda x, **kw: x):
        res = await update_subscription_topics(payload, mock_request, mock_db, mock_user)
...
async def test_get_push_topics(mock_db, mock_user):
    record = models.UserPushTopic(user_id=mock_user.id, topics=["news"], updated_at=datetime.now(UTC))
    mock_db.execute.return_value = MagicMock(scalar_one_or_none=MagicMock(return_value=record))
    
    with patch("app.routers.notifications.get_allowed_topics", return_value=["news", "sports"]), \
...
async def test_send_test_push(mock_db, mock_admin, mock_user, mock_request):
    ...
    sub = models.PushSubscription(id=1, user_id=mock_user.id, endpoint="https://push.com/123", topics=["system"])
    ...
    with patch("app.routers.notifications.enforce_rate_limit", AsyncMock()), \
         patch("app.routers.notifications.subscription_supports_topic", return_value=True), \
         patch("app.routers.notifications._deliver_to_subscription", AsyncMock(return_value=WebPushResult(1, "url", 1, "sent"))):
...
async def test_refresh_user_topic_preferences(mock_db):
    ...
    mock_db.execute.side_effect = [topics_mock, MagicMock(scalar_one_or_none=MagicMock(return_value=None))]
    
    with patch("app.routers.notifications.sort_topics", return_value=["news", "sports"]):
...
def test_build_payload():
    with patch("app.services.webpush.render_notification_template", return_value={"title": "FromTemplate"}):
...
async def test_delivery_to_user(mock_db, mock_user):
    sub = models.PushSubscription(id=1, user_id=mock_user.id, endpoint="url", p256dh="k", auth="s", topics=["news"])
    mock_scalars = MagicMock()
    mock_scalars.all.return_value = [sub]
    mock_res = MagicMock()
    mock_res.scalars.return_value = mock_scalars
    ...
    with patch("app.services.webpush.async_session", return_value=mock_db), \
         ...
         patch("app.services.webpush.asyncio.to_thread", AsyncMock(return_value=WebPushResult(1, "url", 1, "sent"))):
...
async def test_send_test_no_subs(mock_db, mock_admin, mock_user, mock_request):
    ...
    with patch("app.routers.notifications.enforce_rate_limit", AsyncMock()):
        ...
        res = await send_test(mock_request, mock_db, mock_admin, PushTestRequest(user_id=mock_user.id))
        assert res.total == 0
...
async def test_admin_topic_routes(mock_db, mock_admin, mock_request):
    from app.routers.notifications import admin_get_user_topics, admin_update_user_topics, AdminUserTopicsUpdate
    ...
    mock_db.execute.return_value = MagicMock(scalar_one_or_none=MagicMock(return_value=target))
    ...
    with patch("app.routers.notifications.translate", return_value="Error"):
        res = await admin_update_user_topics(999, payload, mock_request, mock_db, mock_admin)
        assert res.user_id == 999
...
async def test_send_web_push_error(mock_db):
    ...
    with patch("app.services.webpush.webpush", side_effect=exc), \
         patch("app.services.webpush._ensure_sync_sessionmaker", return_value=MagicMock()):
...
def test_validators_extra():
    from app.routers.notifications import NotificationAction, NotifyBody, PushSubscriptionKeys, PushSubscriptionIn
    ...
    assert PushSubscriptionIn(endpoint=None, keys={"p256dh":"k","auth":"a"}).endpoint == ""
    # PushSubscriptionIn._normalize_topics
    assert PushSubscriptionIn(endpoint="e", keys={"p256dh":"k","auth":"a"}, topics=None).topics is None
...
def test_build_payload_merge_data():
    with patch("app.services.webpush.render_notification_template", return_value={"data": {"t1": 1}}):
...
async def test_send_to_user_error_paths(mock_db):
    from app.services.webpush import send_to_user
    # Rate limited
    with patch("app.services.webpush.enforce_rate_limit", side_effect=HTTPException(429)): # wait, webpush uses _check_rate_limit
        pass
    
    with patch("app.services.webpush._check_rate_limit", return_value=MagicMock(allowed=False, retry_after=60)):
...
def test_webpush_cleanup():
    ...
    with patch("app.services.webpush._Session", MagicMock()), \
