...
async def test_user_service_mega():
    ...
    with patch("app.services.user_service.save_upload", return_value="/url"), \
         patch("app.services.user_service.delete_static_file", new_callable=AsyncMock) as m_del, \
    ...
    data_user = schemas.UserCreate(email="n@e.com", password="password123", full_name="N", role="teacher", invite_code="inv")
    with patch("app.services.user_service.resolve_locale", return_value="en"):
        # Forbidden
        with pytest.raises(HTTPException) as exc:
    ...
    with patch("app.services.user_service.crud.admin_update_user", return_value=(db_user, MagicMock(changed=True))), \
         patch("app.services.user_service.resolve_locale", return_value="en"), \
         patch("app.services.user_service.create_notifications_for_users", new_callable=AsyncMock) as m_notif:
    ...
    mock_res_sessions.scalars.return_value = [models.ActiveSession(id=1, ip_address="1.1.1.1")]
    mock_res_notifs = MagicMock()
    mock_res_notifs.scalars.return_value = [models.Notification(id=1, title="T")]
    mock_res_logs = MagicMock()
    mock_res_logs.scalars.return_value.all.return_value = [models.DataAccessLog(id=1, resource_type="users", action="access")]
    db.execute.side_effect = [mock_res_sessions, mock_res_notifs, mock_res_logs, MagicMock()] # sessions, notifications, logs, challenges
    
    with patch("app.services.user_service.ensure_mfa_relationships_loaded", new_callable=AsyncMock), \
         patch("app.services.user_service.attach_pending_email", new_callable=AsyncMock), \
         patch("app.schemas.schemas.UserOut.from_orm", return_value=MagicMock(model_dump=lambda: {})):
