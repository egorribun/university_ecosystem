...
from app.localization import normalize_locale, localized_text, translate
from app.utils.files import normalize_filename_prefix, detect_mime_type, save_attachment
...
async def test_monster_coverage_run():
    ...
    with patch("app.crud._is_postgres_session", new_callable=AsyncMock, return_value=False):
        await crud.get_all_events(mock_db, user_id=1, search="s", locale="ru")
    
    # 4. STATS (300 lines)
    with patch("app.services.stats_cache.get_cached_stats", new_callable=AsyncMock, return_value=None), \
         patch("app.services.stats_cache.set_cached_stats", new_callable=AsyncMock):
        # attendance returns result.all()
        row = MagicMock(current_total=1, current_attended=1, previous_total=1, previous_attended=1, rn=1, starts_at=datetime.datetime.now(datetime.UTC), title="T")
        res_attr = MagicMock()
        ...
        notif = models.Notification(body=json.dumps({"score": 5, "course": "C"}), title="T", created_at=datetime.datetime.now(datetime.UTC))
        res_grad = MagicMock()
        ...
        p_row = (1, datetime.datetime.now(datetime.UTC), datetime.datetime.now(datetime.UTC), datetime.datetime.now(datetime.UTC) + datetime.timedelta(hours=2), "T", "T")
        res_part = MagicMock()
    ...
    with patch("app.utils.files._get_storage_backend") as m_be:
        ...
        with patch("app.utils.files.detect_mime_type", return_value="application/pdf"), \
    ...
    with patch("app.auth.auth._active_lockout", new_callable=AsyncMock, return_value=None), \
         ...
         patch("app.auth.auth.ensure_mfa_relationships_loaded", new_callable=AsyncMock, return_value=user), \
         ...
         patch("app.auth.auth.create_access_token", new_callable=AsyncMock, return_value=("token", None)), \
         patch("app.auth.auth._build_token_response", new_callable=AsyncMock, return_value=MagicMock(status_code=200)):

        
        from starlette.responses import Response
        await auth._perform_login("a@b.com", "p", MagicMock(), Response(), mock_db, MagicMock())
