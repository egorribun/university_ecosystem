...
import { Dialog, DialogTitle, DialogContent, TextField, List, ListItem, ListItemButton, ListItemAvatar, Avatar, ListItemText, IconButton } from "@mui/material"
            ...
            {contacts.map((contact) => (
                ...
                >
                    <div className="relative flex-shrink-0">
                        ...
                        />
                        {contact.online && (
                            <span className={`absolute bottom-0 right-0 w-3.5 h-3.5 border-2 rounded-full ${selectedId === contact.id ? 'border-blue-500 bg-white' : 'border-white dark:border-[#0b111e] bg-green-500'}`}></span>
                        )}
                    </div>
                    <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline">
                            <h3 className={`font-semibold text-sm truncate ${selectedId === contact.id ? 'text-white' : 'text-gray-900 dark:text-gray-100'}`}>
                                {contact.name}
                            </h3>
                            <span className={`text-xs ${selectedId === contact.id ? 'text-blue-100' : 'text-gray-500'}`}>
                                {contact.lastMessageTime}
                            </span>
                        </div>
                        <p className={`text-sm truncate ${selectedId === contact.id ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`}>
                            {contact.lastMessage}
                        </p>
                    </div>
                    {contact.unread > 0 && (
...
export const MessageInput: React.FC<MessageInputProps> = ({ onSend }) => {
    ...
    const handleSend = () => {
        console.log('MessageInput handleSend:', { text, filesCount: selectedFiles.length, files: selectedFiles })
