...
export function ScheduleCard({
    ...
    const todayLessons = useMemo(() => {
        return schedule
            .filter((l) => {
                ...
                return (
                    (l.parity === "both" || l.parity === parity) && lessonIndex === todayIndex
                );
            })
    ...
    const nextLesson = useMemo(() => {
        if (currentLesson) {
            ...
            return (
                todayLessons.find((l) => (parseMinutes(l.start_time) ?? 0) > endM) || null
            );
        }
        return (
            todayLessons.find((l) => (parseMinutes(l.start_time) ?? 0) > minutesNow) || null
        );
