...
export const useAuthApi = (
    ...
    const prefetchDashboardData = useCallback(
        async (profileUser: User) => {
            try {
                ...
                const [{ prefetchDashboardStories }, { prefetchDashboardNews }, { prefetchDashboardEvents }, { prefetchEventsListQuery, EVENTS_PAGE_SIZE }] = await Promise.all([
    ...
    const login = useCallback(
        async (
            ...
            try {
                ...
                if (isAxiosError(error) && error.response?.status === 423) {
                    ...
                    if (duration) {
                        throw new Error(t("errors.account_locked_retry", { duration }))
                    }
                    throw new Error(t("errors.account_locked"))
                }
                throw error
            } finally {
        ...
        [authOperation, prefetchDashboardData, setAuthOperation, setUser, t, updatePendingMfa, updateSessionSigningKey]
    )
            ...
            try {
                ...
                if (isAxiosError(error) && error.response?.status === 423) {
                    ...
                    const message = duration
                        ? t("errors.account_locked_retry", { duration })
