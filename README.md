...
- Frontend сборка контролируется плагином Vite, который прерывает build, если размер любого JavaScript-чанка превышает 250 KB.
- В отчёте сборки видны gzip/brotli размеры, а News/Map вынесены в отдельные чанки для более агрессивного code-splitting.
