...
- Map rendering: `react-map-gl/maplibre` (`<Map>`, `<Marker>`, `<Popup>`, `<Source>`, `<Layer>`) — NOT react-leaflet (removed Wave 100)
- Map 3D buildings: `fill-extrusion` layer from OpenFreeMap vector tiles (`source: "openmaptiles"`, `source-layer: "building"`) — render_height data included (DESIGN-100-01)
- Map tiles: OpenFreeMap `https://tiles.openfreemap.org/styles/bright` (light) / `dark` (dark) — native dark theme, no CSS filter hack (DESIGN-100-02)
- Map camera: default pitch 45°, bearing 0°, zoom 16. User can rotate/tilt freely (DESIGN-100-03)
- Map markers: JSX children inside `<Marker>` — NOT `L.divIcon` HTML strings. Popups via `useState<boolean>` conditional render (DESIGN-100-04)
...
- Map ref: `useRef<MapRef>()` from `react-map-gl/maplibre`. `zoomIn()`/`zoomOut()` via `getMap()`, `flyTo()` directly on ref (INFRA-100-01)
- Map dark mode: reactive via `useSyncExternalStore` + `MutationObserver` on `<html class>` — responds to theme toggle instantly (INFRA-100-02)
- Map POI: hardcoded `CAMPUS_POIS` in `campusPOI.ts` + lazy Overpass API via `useOverpassPOI()` (`enabled: false` + manual `refetch()`) (INFRA-100-03)
- Map lazy: `React.lazy(() => import("@/components/map/MapLibreMap"))` — maplibre-gl CSS+JS only loaded in map mode (INFRA-100-04)

## Audit Trail
- Wave 99-100: Campus map — real GUU data + Leaflet→MapLibre GL 3D — 19 files (+2898/-275). Wave 99: 8 real GUU buildings from 2GIS/Wikipedia (ГУК стр.8, Поточные стр.5, Лабораторный стр.4, Админ стр.1, Спорт стр.3, Общ.2 стр.2, Общ.6 к.6, ПНПК стр.16). campusBuildings.ts restructured: BuildingLetter А-З (8), geoCoords field, structureId field, 48 rooms. campusPOI.ts (10 hardcoded POIs). useOverpassPOI.ts (TanStack Query lazy fetch + localStorage 24h cache). overpassQuery.ts (Overpass QL builder + parser). Wave 100: Full Leaflet→MapLibre GL migration. Dependencies: removed leaflet+react-leaflet+@types/leaflet, added maplibre-gl+react-map-gl. MapLibreMap.tsx (react-map-gl/maplibre <Map>+<Layer>+<Marker>+<Popup>). 3D fill-extrusion buildings from OpenFreeMap vector tiles (DESIGN-100-01). Native dark theme via style URL switch (DESIGN-100-02). Default pitch 45° (DESIGN-100-03). JSX markers replacing L.divIcon HTML strings (DESIGN-100-04). Coordinate order fix: geoCoords[0]=lat, [1]=lng→MapLibre longitude/latitude props (FIX-100-01). MapRef from react-map-gl, getMap() for zoomIn/zoomOut (INFRA-100-01). Reactive dark mode via useSyncExternalStore+MutationObserver (INFRA-100-02). POIControls with category filter + load more (INFRA-100-03). React.lazy boundary preserved — MapLibre 272KB gzip lazy only (INFRA-100-04). Russian i18n pluralization _one/_few/_many/_other for roomCount/floors/rooms. buildingIcons.ts extended to 8 colors. map.css Leaflet overrides→MapLibre overrides. Exhaustive audit: 9 UTF-8 corruptions fixed, 2 defaultValue violations removed, CSS variable renamed --map-leaflet-attribution-bg→--map-attribution-bg, all Leaflet comments updated. i18n: 198/204 EN/RU (6 RU-only = plural forms). 0 TS errors, 0 lint errors, 290.92 KB main chunk (unchanged), 0 new console errors.
