...
- Schedule hero: `.sched-hero-card` (4-layer shadow, accent gradient line, corner flare) wraps title+status only — toolbar is OUTSIDE hero card (Wave 72 restructure, DESIGN-71-01)
...
- Schedule toolbar: lives OUTSIDE `sched-hero-card` — hero card = title + status only, toolbar = standalone `FadeSection` below (Wave 72)
