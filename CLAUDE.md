...
- Map buildings: 9 real GUU buildings (А-И), `BuildingLetter` type includes "И". Coords verified via 2GIS/Yandex/OSM Overpass. Floor counts from totadres.ru registry (INFRA-101-01)
...
- Map data files: `campusBuildingFootprints.ts` (GeoJSON polygons, unused but available), `campusWalkways.ts` (GeoJSON paths, unused — approximate data)
