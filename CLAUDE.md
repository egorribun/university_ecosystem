...
- Frontend WebKit axe: `tests/e2e/a11y-public.spec.ts` skips webkit `/login` × 2 themes + all mobile-webkit × 4 cases. Wave 114 SW2a attempted `AxeBuilder.include()` narrowing, `disableRules(["color-contrast", "color-contrast-enhanced"])`, and `@axe-core/playwright` 4.10→4.11.2 upgrade — none unlocked the OOM. Root cause: axe injects the full ruleset context (~200 KB) into document *before* filters apply, and the WebKit renderer exhausts memory on that injection. Wave 115 SW2a-remainder paths: inject a minimised axe bundle via `page.evaluate()`, or conditionally render a reduced `ParticleAuthBackground` in `process.env.NODE_ENV === "test"`. Chromium + Firefox keep the full ruleset, so WCAG 2.2 AA contrast + structural coverage stays green in CI aggregate (A11Y-113-04 partial, Wave 114 SW2a)
- Frontend test file skip pointers: keep the pointer fresh. Anything still `describe.skip` / `it.skip` after Wave 114 SW1 must point to `Wave 115 SW1-remainder` (not `Wave 114 SW1` — that's stale). Current Wave 115 SW1-remainder list: `News.performance` × 2 (stale `[data-page-fade]` assertion), `NewsFeed` (pre-existing Vitest dynamic-import bug), `EventsPagination` (Wave 76 replaced "Load more" with infinite scroll), `Navbar` × 2 (framer mock doesn't transition AnimatePresence exits), `skipLink` dashboard route (framer `useScroll` needs hydrated refs jsdom doesn't provide). Plus `Wave 115 SW2a-remainder` for the 6 WebKit axe cases and `Wave 115 SW2b-remainder` for the 2 /login contrast violations (Wave 114 SW3)
