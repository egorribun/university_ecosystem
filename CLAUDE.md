...
- Map data: `campusBuildings.ts` — structural data (rooms, floors, SVG IDs) is locale-independent; localized names from `map.json` via `getCampusBuildings(locale)` (INFRA-88-01)
- Map i18n: 128/128 keys EN↔RU in `map.json`, namespace `"map"` registered in `metadata.ts` (INFRA-88-02)
- MapFeature: orchestrator in `features/map/MapFeature.tsx` — mirrors EventsFeature pattern (.map-theme, backdrop, z-stacking) (INFRA-89-01)

## Audit Trail
- Wave 97-98: Campus map — polish + visual upgrades — ~14 files modified, 1 new. Phase 1 (Wave 97): CSS tokens (--map-badge-bg, --fs-map-badge, --map-accent-icon, --map-layer-active-bg/text, --map-on-accent, --map-svg-window-opacity, --map-svg-tree/bench). Performance: ref-mirror pattern in useMapNavigation (PERF-97-01) — listeners attach once. A11y: mobile sidebar scroll lock + focus trap + aria-modal (A11Y-97-02), SVG focus ring drop-shadow (A11Y-97-01), search clear i18n, corridor aria-hidden, room glow @keyframes. Hardcoded "white"→token in 4 components. Dead --map-card-glow→applied to hover. Phase 2 (Wave 98): useTimeOfDay hook → data-time-period CSS atmosphere (dawn/morning/afternoon/dusk/night) with dark mode variants (DESIGN-98-01). motion.g building entrance spring stagger (DESIGN-98-02). Richer geometry: window rows, door indicators, 8 trees + 3 benches (DESIGN-98-03). AnimatePresence campus↔floorplan view transition (DESIGN-98-04). i18n: 129/129 EN↔RU (+1 search.clear). 0 TS errors, 0 lint errors, 291 KB main chunk (-3 KB from useMemo), 0 new deps.
