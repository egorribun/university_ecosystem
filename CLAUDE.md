...
- Schedule hero: `.sched-hero-card` (4-layer shadow, accent gradient line, corner flare) wraps title+status only — NO toolbar inside (Wave 72)
- Schedule controls: single `SlidersHorizontal` icon button in hero title row → opens `ScheduleSettingsPanel` with week nav, parity, export, display prefs (Wave 72)
