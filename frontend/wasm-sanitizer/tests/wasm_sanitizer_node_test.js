...
import { initSync, sanitize_rich_text, sanitize_html_basic, strip_html } from '../pkg/wasm_sanitizer.js';
...
const testCases = [
    ...
    { name: 'basic rich text tags', input: '<p>Hello <b>world</b></p><ul><li>item 1</li><li>item 2</li></ul>' },
    { name: 'allowed headings and code', input: '<h1>Title</h1><h3>Subtitle</h3><pre><code>let x = 42;</code></pre>' },
    { name: 'unallowed tags (div, span, img)', input: '<div class="evil"><span id="s1">nested text</span><img src="x" /></div>' },
    { name: 'script tag XSS', input: '<script>alert(1)</script>' },
    { name: 'script tag with attributes', input: '<script type="text/javascript" src="evil.js"></script>' },
    { name: 'event handler attributes', input: '<a href="#" onclick="runEvil()">click me</a><b onerror="alert(1)">bold</b>' },
    { name: 'javascript protocol href', input: '<a href="javascript:alert(1)">javascript link</a>' },
    { name: 'data protocol href', input: '<a href="data:text/html,<script>alert(1)</script>">data link</a>' },
    { name: 'safe https href and rel', input: '<a href="https://google.com" target="_blank" title="Google">Safe Link</a>' },
    ...
    { name: 'Deep tag nesting (150 levels)', input: '<div>'.repeat(150) + 'content' + '</div>'.repeat(150) },
    { name: 'Malformed HTML tags', input: '<p class="unclosed' },
    { name: 'Special characters', input: 'Text & symbols < > " \' &amp;' },
    { name: 'Disallowed attributes on allowed tags', input: '<p class="class1" id="p1" style="font-size: 12px;">paragraph</p>' },
    ...
    { name: 'Only inline basic elements', input: '<b>bold</b> and <i>italic</i> and <strong>strong</strong>' },
    { name: 'Basic mode stripping rich elements', input: '<h1>Header</h1><a href="https://example.com">link</a>' }
];
