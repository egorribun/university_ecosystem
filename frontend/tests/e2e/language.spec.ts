...
test.describe("Language switching and document metadata", () => {
  ...
  test("switches locales, direction and translations", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /Язык интерфейса|Language/i }).click()
    const english = page.getByRole("radio", { name: /Английский|English/i })
    await english.focus()
    ...
    const russian = page.getByRole("radio", { name: /Русский|Russian/i })
    await russian.focus()
    await page.keyboard.press("Space")
