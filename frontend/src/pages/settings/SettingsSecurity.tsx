            ...
            <div className="flex flex-col gap-4">
               {totpError ? (
                ...
                {activeTotp.map((enrollment) => (
                  <div key={enrollment.id} className="flex items-center justify-between p-3 rounded-lg bg-(--bg-surface-hover) border border-(--border-subtle)">
                      <div className="flex items-center gap-3">
                        <Smartphone className="w-5 h-5 text-(--text-secondary)" />
                        <div className="flex flex-col">
                           <span className="font-semibold text-sm">{enrollment.label || t("settings:security.mfa.totp.defaultLabel", "Authenticator App")}</span>
                           <span className="text-xs text-(--text-secondary)">
                             {t("settings:security.mfa.totp.addedOn", { date: formatDateTime(enrollment.created_at) })}
                           </span>
                        </div>
                      </div>
                      <Button
                  ...
                  <>
                    <div className="flex flex-col gap-2">
                       {webauthnCredentials.map((cred) => (
                          <div key={cred.id} className="flex items-center justify-between p-3 rounded-lg bg-(--bg-surface-hover) border border-(--border-subtle)">
                              <div className="flex items-center gap-3">
                                <Fingerprint className="w-5 h-5 text-(--text-secondary)" />
                                <div className="flex flex-col">
                                   <span className="font-semibold text-sm">{cred.label || t("settings:security.mfa.webauthn.defaultLabel", "Passkey")}</span>
                                    <span className="text-xs text-(--text-secondary)">
                                     {t("settings:security.mfa.webauthn.addedOn", { date: formatDateTime(cred.created_at) })}
                                   </span>
                                </div>
                              </div>
                              <Button
