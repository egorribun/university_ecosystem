...
describe("Runtime utility behavior", () => {
  ...
  describe("bootstrapFallback.ts", () => {
    ...
    it("reloads when service workers and Cache Storage are unavailable", async () => {
      const mockReload = vi.fn()
      const fakeWindow = {
        location: { reload: mockReload },
        navigator: {},
      ...
      const rendered = renderBootstrapFallback({
        documentRef: customDoc,
        rootElement: mockRoot,
        copy: getBootstrapFallbackCopy(customDoc),
        logError: vi.fn(),
      })
  ...
  describe("performance.ts", () => {
    ...
    it("getWebVitals returns structure and handles navigation entry ttfb and fcp", () => {
      ...
      spyType.mockReturnValue([])
      spyName.mockReturnValue([])
      expect(getWebVitals()).toEqual({})
    ...
    it("reportMetric log/otel", () => {
      ...
      vi.unstubAllGlobals()
      expect(() => reportMetric("without-otel", 1)).not.toThrow()
    })

    it("keeps development-only logging disabled in production", async () => {
      vi.stubEnv("DEV", false)
      vi.resetModules()
      try {
        const productionPerformance = await import("../performance")
        await expect(
          productionPerformance.measureAsync("production", async () => "ok")
        ).resolves.toBe("ok")
        expect(() => productionPerformance.reportMetric("production", 1)).not.toThrow()
      } finally {
        vi.unstubAllEnvs()
      }
    })
    ...
    it("flushes from its interval and permits repeated stop calls", () => {
      vi.useFakeTimers()
      try {
        const freshBuffer = new (metricsBuffer as any).constructor()
        freshBuffer.record("interval-metric", 1)
        freshBuffer.start(10)
