    ...
    <div className="w-full min-h-screen bg-page py-8 sm:py-10">
      <div className="px-2 md:px-4">
        ...
        <div className="hidden md:block">
          {/* Desktop Table Mock */}
          <div className="rounded-lg border border-glass-border bg-surface/(--opacity-medium) p-4 backdrop-blur-md shadow-glass">
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="flex gap-4">
                  {Array.from({ length: 7 }).map((_, j) => (
                    <Skeleton key={j} className="flex-1 h-24" rounded="md" />
                  ))}
                </div>
              ))}
            </div>
        ...
        <div className="md:hidden space-y-6">
          ...
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="space-y-3">
              ...
              <Skeleton width="100%" height={100} rounded="radius-2xl" />
              <Skeleton width="100%" height={100} rounded="radius-2xl" />
            </div>
          ))}
        </div>
